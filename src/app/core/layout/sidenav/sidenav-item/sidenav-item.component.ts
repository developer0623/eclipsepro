...
export class SidenavItemComponent implements OnInit {
  ...
  constructor(private sidenavService: SidenavService, private router: Router, private featureFlagService: FeatureFlagService) {
  ...
  ngOnInit() {
    if (this.item && this.item.featureFlag) {
      this.featureFlagService.getFeatureFlags().subscribe((flags) => {
        this.isFeatureShow = flags[this.item.featureFlag] !== false;
      });
    }
  }
