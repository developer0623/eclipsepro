import {
  ActionReducerMap,
  createSelector,
  createFeatureSelector,
  ActionReducer,
  MetaReducer,
} from '@ngrx/store';
import { environment } from '../../environments/environment';
import * as fromRouter from '@ngrx/router-store';

/**
 * storeFreeze prevents state from being mutated. When mutation occurs, an
 * exception will be thrown. This is useful during development mode to
 * ensure that none of the reducers accidentally mutates the state.
 */
import { storeFreeze } from 'ngrx-store-freeze';

/**
 * Every reducer module's default export is the reducer function itself. In
 * addition, each module should export a type or interface that describes
 * the state of the reducer plus any selector functions. The `* as`
 * notation packages up all of the exports into a single object.
 */

import * as fromDashboard from './dashboard/reducers';
import * as fromSchedule from './scheduler/reducers';
import * as fromSignalr from './signalr/reducers';
import * as fromAlerts from './alerts/reducers';
import * as fromSettings from './settings/reducers';
import * as fromInventory from './inventory/reducers';
import * as fromWarehouse from './warehouse/reducers';
import * as fromWReports from './reports/reducers';
import * as fromSubscriptions from './subscriptions/reducers';
import * as fromProductionExplorer from './productionExplorer/reducers';

/**
 * As mentioned, we treat each reducer like a table in a database. This means
 * our top level state interface is just a map of keys to inner state types.
 */
export interface RootState {
  router: fromRouter.RouterReducerState;
  dashboard: fromDashboard.State;
  schedule: fromSchedule.State;
  signalRConnection: fromSignalr.State;
  alerts: fromAlerts.State;
  settings: fromSettings.State;
  inventory: fromInventory.State;
  warehouse: fromWarehouse.State;
  reports: fromWReports.State;
  subscriptions: fromSubscriptions.State;
  productionExplorer: fromProductionExplorer.State;
}

/**
 * Our state is composed of a map of action reducer functions.
 * These reducer functions are called with each dispatched action
 * and the current or initial state and return a new immutable state.
 */
export const reducers: ActionReducerMap<RootState> = {
  router: fromRouter.routerReducer,
  dashboard: fromDashboard.reducer,
  schedule: fromSchedule.reducer,
  signalRConnection: fromSignalr.reducer,
  alerts: fromAlerts.reducer,
  settings: fromSettings.reducer,
  inventory: fromInventory.reducer,
  warehouse: fromWarehouse.reducer,
  reports: fromWReports.reducer,
  subscriptions: fromSubscriptions.reducer,
  productionExplorer: fromProductionExplorer.reducer
};

// console.log all actions
export function logger(reducer: ActionReducer<RootState>): ActionReducer<RootState> {
  return (state: RootState, action: any): any => {
    const result = reducer(state, action);
    console.groupCollapsed(action.type);
    console.log('prev state', state);
    console.log('action', action);
    console.log('next state', result);
    console.groupEnd();

    return result;
  };
}

/**
 * By default, @ngrx/store uses combineReducers with the reducer map to compose
 * the root meta-reducer. To add more meta-reducers, provide an array of meta-reducers
 * that will be composed to form the root meta-reducer.
 */
export const metaReducers: MetaReducer<RootState>[] = !environment.production
  ? [logger, storeFreeze]
  : [];

