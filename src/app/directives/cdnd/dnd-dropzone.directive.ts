...
export class DndDropzoneDirective implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    ...
    if (this.dndDropGroup) {
      this.getItems(this.dndDropGroup);
    }
  ...
  onDragOver( event: DragEvent ) {
    ...
    const positionPlaceholderBeforeDirectChild =
      shouldPositionPlaceholderBeforeElement( event, this.elementRef.nativeElement, this.dndHorizontal );

    switch (this.dndAreaType) {
      case 'header': {
        if (!positionPlaceholderBeforeDirectChild) {
          this.dropPosFlag = 1;
          this.elementRef.nativeElement.style.borderLeftWidth = '0px';
          this.elementRef.nativeElement.style.borderRightWidth = '2px';
        } else {
          this.dropPosFlag = 0;
          this.elementRef.nativeElement.style.borderLeftWidth = '2px';
          this.elementRef.nativeElement.style.borderRightWidth = '0px';
        }
        break;
        ...
        if (positionPlaceholderBeforeDirectChild) {
          this.dropPosFlag = 0;
          this.elementRef.nativeElement.style.borderTopWidth = '2px';
          this.elementRef.nativeElement.style.borderBottomWidth = '0px';
        } else {
          this.dropPosFlag = 1;
          this.elementRef.nativeElement.style.borderTopWidth = '0px';
          this.elementRef.nativeElement.style.borderBottomWidth = '2px';
        }
        break;
      } case 'machine': {
        this.elementRef.nativeElement.children[0].style.background = 'rgba(204, 204, 204, 0.7)';
        this.elementRef.nativeElement.children[0].style.color = '#ffffff';
      }
    }
  ...
  private getDropIndex(): number {
    if (this.groupItems.length === 0) {
      return 0;
    }
    if (!this.dropPosFlag) {
      return this.groupItems[0].sequenceNum;
    }
    return this.groupItems[this.groupItems.length - 1].sequenceNum + this.dropPosFlag;
  }

  private cleanupDragoverState() {
    if (this.dndAreaType === 'machine') {
      this.elementRef.nativeElement.children[0].style.background = 'rgba(255, 255, 255, 0)';
      this.elementRef.nativeElement.children[0].style.color = 'transparent';
    } else {
      this.elementRef.nativeElement.style.borderWidth = '0px';
    }
